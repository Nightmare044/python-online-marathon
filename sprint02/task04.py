...
Using regular expression write method max_population() for parsing strings and get info
 about location with highest population """

import re


def max_population(data):
    p = max([(re.findall(r"(\w*),\d*,\w$", i)[0], int(re.findall(r"(\d*),\w$", i)[0]))
             for n, i in enumerate(data) if n > 0], key=lambda x: int(x[1]))
    return p
